...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <form action="{{ route('voyager.jobs.update', $job->id) }}" method="post" id="job-form">
            @csrf
            @method('PUT') {{-- Voyager typically uses PUT for updates --}}
            ...
            <div class="card p-3 mb-3">
                <div class="row g-3 align-items-end">
                    ...
                    <div class="col-md-3">
                        ...
                        <select name="is_active" class="form-select">
                            @php
                                $isActiveOld = (string) old('is_active', (string) (int) $job->is_active);
                            @endphp
                            <option value="1" @selected($isActiveOld === '1')>Yes</option>
                            <option value="0" @selected($isActiveOld === '0')>No</option>
                        </select>
                    </div>

                    <div class="col-md-3">
                        <label class="form-label">Company</label>
                        {{-- Display only: saved against the current company context --}}
                ...
                <div class="row g-3 mt-2">
                    <div class="col-md-12 small text-muted">
                        Created at: {{ optional($job->created_at)->format('Y-m-d H:i') ?? '—' }} —
                        Last updated: {{ optional($job->updated_at)->format('Y-m-d H:i') ?? '—' }}
                    </div>
                </div>
            </div>
...
@section('js_scripts')
    ...
    <script>
        // Select2 defaults (LTR, Bootstrap 5)
        if ($.fn.select2) {
