...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <div class="d-flex justify-content-between align-items-center mb-3">
            <h4 class="m-0">Edit Project #{{ $project->id }}</h4>
        ...
        <form action="{{ route('voyager.site_projects.update', $project->id) }}" method="POST">
            @csrf
            @method('PUT')
                ...
                <div class="card-body row g-3">
                    ...
                    <div class="col-md-3">
                        ...
                        <input type="number" name="display_order" class="form-control"
                               value="{{ old('display_order',$project->display_order) }}" min="0">
                    </div>
                    <div class="col-md-12">
            ...
            <div class="card form-card mb-3">
                ...
                <div class="card-body">
                    <textarea name="body" class="form-control tinymce"
                              rows="10">{{ old('body', $project->body) }}</textarea>
                </div>
            </div>
            ...
            <div class="card form-card mb-3">
                ...
                <div class="card-body" id="imagesContainer">
                    ...
                    @foreach(old('images', $project->images->toArray()) as $img)
                        <div class="image-row row g-2 align-items-end">
                            <div class="col-md-1 d-none d-md-block">
                                @if(($img['image_path'] ?? null))
                                    <img src="{{ $img['image_path'] }}" class="thumb" alt="">
                                @endif
                            </div>
                            <div class="col-md-4">
                                <label class="form-label">Image Path</label>
                                <input type="text" name="images[{{ $imgIndex }}][image_path]" class="form-control"
                                       value="{{ $img['image_path'] ?? '' }}" placeholder="/uploads/projects/img.jpg">
                                <input type="hidden" name="images[{{ $imgIndex }}][id]" value="{{ $img['id'] ?? '' }}">
                            </div>
                            <div class="col-md-4">
                                <label class="form-label">Caption</label>
                                <input type="text" name="images[{{ $imgIndex }}][caption]" class="form-control"
                                       maxlength="255" value="{{ $img['caption'] ?? '' }}">
                            </div>
                            <div class="col-md-2">
                                <label class="form-label">Order</label>
                                <input type="number" name="images[{{ $imgIndex }}][display_order]" class="form-control"
                                       value="{{ $img['display_order'] ?? 0 }}" min="0">
                            </div>
                            <div class="col-md-1 text-end">
                                <label class="form-label">Cover</label>
                                <input type="checkbox" name="images[{{ $imgIndex }}][is_cover]" value="1"
                                       class="form-check-input" @checked(!empty($img['is_cover']))>
                            </div>
            ...
            <div class="card form-card mb-3">
                ...
                <div class="card-body" id="featuresContainer">
                    @php $featIndex = 0; @endphp
                    @foreach(old('features', $project->features->toArray()) as $feat)
                        <div class="feature-row row g-2 align-items-end">
                            <div class="col-md-10">
                                <label class="form-label">Feature</label>
                                <input type="text" name="features[{{ $featIndex }}][label]" class="form-control"
                                       value="{{ $feat['label'] ?? '' }}" placeholder="e.g., Sustainable materials">
                                <input type="hidden" name="features[{{ $featIndex }}][id]"
                                       value="{{ $feat['id'] ?? '' }}">
                            </div>
                            <div class="col-md-2">
                                <label class="form-label">Order</label>
                                <input type="number" name="features[{{ $featIndex }}][display_order]"
                                       class="form-control" value="{{ $feat['display_order'] ?? 0 }}" min="0">
                            </div>
                            <div class="col-12 text-end">
                                <span class="remove-btn small">Remove</span>
                            </div>
                        </div>
                        @php $featIndex++; @endphp
                    @endforeach
                </div>
            </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            document.getElementById('addImageRow').addEventListener('click', function () {
                const idx = nextIdx(imgContainer);
        ...
        `);
            ...
            document.getElementById('addFeatureRow').addEventListener('click', function () {
                const idx = nextIdx(featureContainer);
