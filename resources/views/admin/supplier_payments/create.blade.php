...
@section('content')
    <div class="container-xxl py-3">
        <div class="d-flex justify-content-between align-items-center mb-3">
            <h4 class="m-0">Add Supplier Payment</h4>
        ...
        <form action="{{ route('voyager.supplier_payments.store') }}" method="POST" enctype="multipart/form-data"
              class="card p-3">
            @csrf
            <div class="row g-3">
                <div class="col-md-6">
                ...
                <div class="col-md-6">
                    ...
                    <select name="supplier_material_id" class="form-select js-select2-material">
                        ...
                        @foreach($materials as $m)
                            <option value="{{ $m->id }}"
                                    data-supplier="{{ $m->supplier_id }}" @selected(old('supplier_material_id')==$m->id)>
                                {{ $m->title }} — {{ $m->supplier?->name }}
                            </option>
                        @endforeach
                    </select>
                    <small class="text-muted">Tip: choose supplier first to filter this list.</small>
                </div>
                ...
                <div class="col-md-3">
                    ...
                    <select name="payment_type" class="form-select" required>
                        @foreach($types as $k=>$v)
                            <option value="{{ $k }}" @selected(old('payment_type',$k)=='{{ $k }}')>{{ $v }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="col-md-6">
                    ...
                    <select name="paid_by_employee_id" class="form-select js-select2-employee">
                        ...
                        @foreach($employees as $e)
                            <option value="{{ $e->id }}" @selected(old('paid_by_employee_id')==$e->id)>{{ $e->name }}</option>
                        @endforeach
                    </select>
                </div>
    ...
    <script>
        $(function () {
            ...
            function filterMaterialsBySupplier() {
                ...
                // If current selected material doesn't match, reset
