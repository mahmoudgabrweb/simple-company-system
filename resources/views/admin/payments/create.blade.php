...
@section('content')
    <div class="container-xxl py-3">
        <div class="d-flex justify-content-between align-items-center mb-3">
            <h4 class="m-0">إضافة دفعة</h4>
            <a href="{{ route('voyager.payments.index') }}" class="btn btn-secondary">رجوع</a>
        </div>
        ...
        <form action="{{ route('voyager.payments.store') }}" method="post" enctype="multipart/form-data" class="card p-3">
            @csrf
            <div class="row g-3">
                <div class="col-md-5">
                ...
                <div class="col-md-3">
                    <label class="form-label">من دافع؟</label>
                    <input name="paid_by" class="form-control" value="{{ old('paid_by') }}">
                </div>
                <div class="col-md-4">
                    <label class="form-label">المستلم (موظف)</label>
                    <select name="received_by_employee_id" class="form-select">
                        <option value="">—</option>
                        @foreach($employees as $e)
                            <option value="{{ $e->id }}" @selected(old('received_by_employee_id')==$e->id)>{{ $e->name }}
                                — {{ $e->phone }}</option>
                        @endforeach
                    </select>
                </div>
