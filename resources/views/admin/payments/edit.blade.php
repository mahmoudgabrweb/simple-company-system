...
@section('content')
    <div class="container-xxl py-3">
        <div class="d-flex justify-content-between align-items-center mb-3">
            <h4 class="m-0">تعديل دفعة #{{ $payment->id }}</h4>
                ...
                <div class="col-md-4">
                    ...
                    <select name="received_by_employee_id" class="form-select">
                        ...
                        @foreach($employees as $e)
                            <option value="{{ $e->id }}" @selected($payment->received_by_employee_id==$e->id)>{{ $e->name }}
                                — {{ $e->phone }}</option>
                        @endforeach
                    </select>
                </div>
                    ...
                    @if($payment->attachment_path)
                        <div class="mt-1">
                            <a href="{{ route('admin.payments.attachment', $payment->id) }}"
                               class="btn btn-sm btn-outline-secondary">تنزيل المرفق الحالي</a>
                        </div>
                    @endif
                </div>
            </div>
